    ...
    *****************************************************************
    ...
    public void deleteAccount() {
        ...
        try {
            ...
            // Delete all of the user videos associated with the signed-in user whose primary key is userPrimaryKey
...
//        // Obtain the List of files that belongs to the user with primaryKey
        List<UserVideo> userVideosList = userVideoFacade.findUserVideosByUserPrimaryKey(primaryKey);

        if (!userVideosList.isEmpty()) {
            // Java 8 looping over a list with lambda
            userVideosList.forEach(userFile -> {
                // Remove the user's video record from the database
                userVideoFacade.remove(userFile);
            });
        }
    }
}
